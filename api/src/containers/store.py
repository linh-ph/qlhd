...
def store(data, context, time_start):
    ...
    if len(data['search']) > 0:
        ...
        # Thực hiện select
        s = Image.objects.all().filter(query_1 | query_2).order_by(
            order_by + 'id')[
            start: to]
        c = s.count()
    else:
        ...
        # Thực hiện select
        s = Image.objects.all().filter(status="1", id_user=data['user_id']).order_by(
            order_by + 'id')[
            start: to]
        c = Image.objects.filter(status="1", id_user=data['user_id']).count()
