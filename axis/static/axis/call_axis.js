        ...
        if (response.error) {
          ...
          Swal.fire({
            ...
            html:
              ...
              "<textarea rows='4' cols='50' id='swal-text' class='swal2-input' placeholder='Nhập văn bản...'>"+ text_raw +"</textarea>",
...
function showModalEditByID(id,label){
    console.log(id)
    input = $('#text_ocr_'+id).attr('id')
    text = $('#'+input).val()
    console.log(text)
    ...
    Swal.fire({
        ...
        html:
          ...
          label +
          "' placeholder='Nhập nhãn dán...' required>" +
          "<textarea rows='4' cols='50' id='swal-text' class='swal2-input' placeholder='Nhập văn bản...'>"+ text +"</textarea>",
        showCancelButton: true,
        cancelButtonText: "Bỏ",
        confirmButtonText: "Lưu",
        showLoaderOnConfirm: true,
...
function createTableAxis(response) {
  ...
  for (i = 0; i < table.length; i++) {
    html =
      ...
      "<div class='row'>"+
      "<div>"+
      "<input type='hidden' id='text_ocr_"+table[i].id+"' name='fname_"+i+"' value='"+table[i].text+"'>"+
      "<i class='fas fa-edit' onClick='showModalEditByID("+table[i].id+", \""+table[i].label+"\")'></i>" +
      "</div>"+
